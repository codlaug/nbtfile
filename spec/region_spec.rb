...
describe NBTFile::RegionFile do
  ...
  it "does not immediately create an empty file" do
    expect(File.exists?(@region_filename)).to be_falsey
  end
  ...
  it "creates the file after a chunk has been stored" do
    content = "foobar"
    @region_file.store_chunk(0, 0, content)
    expect(File.exists?(@region_filename)).to be_truthy
  end
...
describe NBTFile::RegionManager do
  ...
  it "creates appropriate region files" do
    ...
    expect(File.exists?(File.join(@temp_dir, 'r.0.0.mcr'))).to be_truthy
    @region_manager.store_chunk(32, 32, content)
    expect(File.exists?(File.join(@temp_dir, 'r.1.1.mcr'))).to be_truthy
    @region_manager.store_chunk(-16, 64, content)
    @region_manager.store_chunk(-32, 64, content)
    expect(File.exists?(File.join(@temp_dir, 'r.-1.2.mcr'))).to be_truthy
    expect(File.exists?(File.join(@temp_dir, 'r.-2.2.mcr'))).to be_falsey
  end
  ...
  it "deletes chunks (idempotently)" do
    ...
    expect(File.exists?(File.join(@temp_dir, 'r.0.0.mcr'))).to be_falsey
    @region_manager.delete_chunk(0, 0)
  end
end
